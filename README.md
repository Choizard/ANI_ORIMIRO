...
### 1. 데이터 분석
#### 1) 데이터양의 미흡
> 2019년 전보다 더 많은 데이터가 있었다면 좋았겠다. 업종별 최대 데이터가 12개라 아쉬운점이 많았다.

#### 2) 다양한 성격의 데이터 수집
> 매출데이터만으로 모델을 만들었기 때문에 비슷한 성격 때문에 다중공선성을 해결하지 못했다. 대중교통 승하차인원과 유동인구 등의 다른 성격의 데이터를 함께 이용했다면 더 좋은 모델을 만들 수 있었을 것
